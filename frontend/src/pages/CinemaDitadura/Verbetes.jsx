...
export default function Verbetes() {
  ...
  return <main className="bg-background py-20 lg:py-28"><Container>
    ...
    {entries.length > 0 && <>
      ...
      <div className="mt-6 grid gap-6 md:grid-cols-2 lg:grid-cols-3">{visibleEntries.map((entry) => <article key={entry.id} role="button" tabIndex={0} aria-label={`Ler verbete ${entry.title}`} onClick={() => setSelectedEntry(entry)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); setSelectedEntry(entry); } }} className="flex cursor-pointer flex-col rounded-2xl border border-border bg-card p-7 transition hover:-translate-y-1 hover:border-primary focus-visible:-translate-y-1 focus-visible:border-primary focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary"><BookOpenText className="text-primary" aria-hidden="true" /><h2 className="mt-5 font-title text-4xl">{entry.title}</h2><p className="mt-4 flex-1 leading-7 text-muted">{limitWords(entry.description || "Verbete em preparação.", 45)}</p><p className="mt-5 text-sm text-muted">Autoria: <strong className="text-text">{entry.researcherName}</strong></p><span className="mt-6 inline-flex items-center gap-2 self-start font-semibold text-primary">Ler verbete <ArrowRight size={16} /></span></article>)}</div>
    </>}
