...
const events = [
  {
    ...
    description:
      "A VII Mostra Cinema e Ditadura integra as atividades do LACE com filmes, transmissoes gravadas, calendario de exibicoes e debates sobre memoria, resistencia e ditadura empresarial-militar.",
    links: [
      {
        label: "Abrir VII Mostra",
...
export default function Eventos2025() {
    ...
    <main className="bg-background py-20 lg:py-28">
      <Container>
        <header className="max-w-4xl">
          ...
          <h1 className="mt-4 font-title text-5xl leading-tight md:text-7xl">Eventos 2025</h1>
        </header>

        <section className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-3" aria-label="Eventos 2025">
          {events.map((event) => (
            <button
              key={event.title}
              type="button"
      ...
      <div className="relative max-h-[94vh] w-full max-w-6xl overflow-y-auto rounded-3xl border border-white/20 bg-background p-5 shadow-2xl md:p-8">
        ...
        <section className="grid gap-8 pr-0 lg:grid-cols-[0.9fr_1.1fr] lg:pr-8">
          ...
          <div>
            ...
            <SocialShare title={event.title} url="/eventos/2025" className="mt-8" />
          </div>
        </section>
      </div>
    </div>,
