...
export default function ResourceFields({ items, onAdd, onRemove, onChange }) {
  return (
    <RepeatableGroup
      title="Documentos, vídeos e links"
      description="Exemplo: Informe Público. Adicione quantos recursos forem necessários."
      items={items}
      onAdd={onAdd}
      onRemove={onRemove}
      addLabel="Recurso"
      renderItem={(item, index) => (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="font-semibold">Tipo<select className={fieldClass} value={item.kind} onChange={(event) => onChange(index, "kind", event.target.value)}><option value="DOCUMENT">Documento</option><option value="VIDEO">Vídeo</option><option value="AUDIO">Áudio</option><option value="LINK">Outro link</option></select></label>
