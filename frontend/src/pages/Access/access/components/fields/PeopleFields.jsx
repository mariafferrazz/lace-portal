    ...
    <RepeatableGroup
      ...
      renderItem={(person, index) => (
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="font-semibold">Funcao ou titulo<input className={fieldClass} placeholder="Pesquisador, coordenacao, doutorando..." value={person.role} onChange={(event) => onChange(index, "role", event.target.value)} /></label>
          <label className="font-semibold md:col-span-2">Descricao<textarea className={`${fieldClass} min-h-28 resize-y`} value={person.description} onChange={(event) => onChange(index, "description", event.target.value)} /></label>
