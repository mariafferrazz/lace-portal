  ...
  return (
    <section className={sectionClass}>
      <div className="grid gap-5 md:grid-cols-2">
        <label className="font-semibold md:col-span-2">Link principal do podcast<input className={fieldClass} type="url" placeholder="Spotify, YouTube ou player público" value={form.podcastUrl} onChange={actions.update("podcastUrl")} /></label>
        ...
        <label className="font-semibold md:col-span-2">Descrição<textarea className={`${fieldClass} min-h-36 resize-y`} value={form.description} onChange={actions.update("description")} /></label>
      </div>
      <div className="mt-5 space-y-5">
        <RepeatableGroup title="Episódios" description="O botão “Abrir episódios” no site usa esta lista. Novos episódios podem ser acrescentados durante a produção." items={form.episodes} onAdd={() => actions.addObject("episodes", actions.emptyEpisode)} onRemove={(index) => actions.removeObject("episodes", index, actions.emptyEpisode)} addLabel="Episódio" renderItem={(episode, index) => <div className="grid gap-4 md:grid-cols-2"><label className="font-semibold">Título<input className={fieldClass} value={episode.title} onChange={(event) => actions.updateObject("episodes", index, "title", event.target.value)} /></label><label className="font-semibold">Link<input className={fieldClass} type="url" placeholder="https://..." value={episode.url} onChange={(event) => actions.updateObject("episodes", index, "url", event.target.value)} /></label><label className="font-semibold md:col-span-2">Descrição<textarea className={`${fieldClass} min-h-24 resize-y`} value={episode.description} onChange={(event) => actions.updateObject("episodes", index, "description", event.target.value)} /></label></div>} />
