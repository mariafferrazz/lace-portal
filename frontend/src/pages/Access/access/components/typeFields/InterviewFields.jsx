  ...
  return (
    <section className={sectionClass}>
      <div className="grid gap-5 md:grid-cols-2">
        <label className="font-semibold md:col-span-2">Link do vídeo<input className={fieldClass} type="url" placeholder="YouTube, Vimeo ou outro player público" value={form.videoUrl} onChange={actions.update("videoUrl")} /></label>
      ...
      <div className="mt-5 space-y-5">
        <PeopleFields title="Pesquisadores e participantes" description="Cadastre nome, função, descrição e Lattes. Campos vazios serão ignorados." items={form.people} onAdd={() => actions.addObject("people", actions.emptyPerson)} onRemove={(index) => actions.removeObject("people", index, actions.emptyPerson)} onChange={(index, field, value) => actions.updateObject("people", index, field, value)} />
