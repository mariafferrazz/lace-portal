    ...
    <section className={sectionClass}>
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-primary/30 bg-background p-4 font-semibold transition hover:border-primary">
          ...
          <span>Criar pagina em Cinema e Ditadura<span className="mt-1 block text-sm font-normal leading-6 text-muted">O card em Eventos apontara para esta pagina, em vez de abrir somente um modal.</span></span>
        </label>
        ...
        <RepeatableGroup
          ...
          renderItem={(session, index) => (
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <label className="font-semibold md:col-span-2">Titulo do filme/sessao<input className={fieldClass} placeholder="Nome do filme" value={session.title} onChange={(event) => actions.updateObject("sessions", index, "title", event.target.value)} /></label>
              ...
              {!session.filmId && (
                <label className="flex cursor-pointer items-start gap-3 rounded-2xl border border-primary/30 bg-background p-4 font-semibold md:col-span-2">
                  ...
                  <span>Adicionar esse filme ao banco de dados<span className="mt-1 block text-sm font-normal text-muted">O filme sera cadastrado no acervo e vinculado automaticamente a esta sessao.</span></span>
                </label>
              )}
              <label className="font-semibold md:col-span-2">Direcao ou debate<input className={fieldClass} value={session.direction} onChange={(event) => actions.updateObject("sessions", index, "direction", event.target.value)} /></label>
              <MultiUrlField className="md:col-span-2" label="Link do video da sessao" values={session.sessionUrls} onChange={(urlIndex, value) => actions.updateNestedUrlList("sessions", index, "sessionUrls", urlIndex, value)} onAdd={() => actions.addNestedUrl("sessions", index, "sessionUrls")} onRemove={(urlIndex) => actions.removeNestedUrl("sessions", index, "sessionUrls", urlIndex)} />
            </div>
          )}
        />
      </div>
