...
export default function ContentPreviewDetails({ content }) {
  ...
  return <div className="mt-6 space-y-6">
    {content.description && <section className="rounded-2xl border border-border bg-card p-5"><h3 className="font-title text-2xl">Descricao</h3><div className="mt-3 whitespace-pre-line leading-7 text-muted">{content.description}</div></section>}
    ...
    <LinkList title="Midia principal" urls={[metadata.videoUrl, metadata.podcastUrl, metadata.pdfUrl, content.externalUrl, content.fileUrl]} />
    <ObjectCards title="Episodios" items={metadata.episodes} render={(item) => <><strong>{item.title || "Sem titulo"}</strong>{item.description && <p className="mt-2 text-muted">{item.description}</p>}{item.url && <a className="mt-3 inline-block text-primary" href={item.url} target="_blank" rel="noreferrer">Abrir episodio</a>}</>} />
    ...
    <ObjectCards title="Informacoes adicionais" items={metadata.credits || metadata.additionalInfo} render={(item) => <><strong>{item.title || item.value || "Informacao"}</strong>{item.value && item.title && <p className="mt-1 text-text">{item.value}</p>}{item.description && <p className="mt-2 text-muted">{item.description}</p>}</>} />
    ...
    <ObjectCards title="Sessoes" items={metadata.sessions} render={(item, index) => <><strong>Sessao {index + 1}: {item.film?.title || item.title || "Sem titulo"}</strong><p className="mt-1 text-sm text-muted">{item.date || "Sem data"}</p><LinkList title="Links da sessao" urls={item.sessionUrls || [item.sessionUrl]} /></>} />
  </div>;
}
